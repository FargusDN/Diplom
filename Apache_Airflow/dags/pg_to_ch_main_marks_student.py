    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            select
                ...
                join {os.getenv('POSTGRES_SOURCE_SCHEMA', 'public')}.main_marks_student tms on tms.user_id = ui.user_id and ui.user_status = 'Студент' 
...
def pg_to_pg_load_data_to_target(**context) -> None:
    ...
    try:
        ...
        # Определяем колонки для обновления (все кроме main_mark_id)
        update_columns = [col for col in columns if col != 'main_mark_id']
        ...
        with conn.cursor() as cursor:
            ...
            upsert_query = f"""
            INSERT INTO {os.getenv('POSTGRES_TARGET_SCHEMA', 'sandbox')}.main_marks_student_for_click
            ({', '.join(columns)})
            VALUES ({', '.join(['%s'] * len(columns))})
            ON CONFLICT (main_mark_id) DO UPDATE SET
            {update_set}
            """
    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            SELECT
                ...
                create_dttm,
                change_dttm
            FROM {os.getenv('POSTGRES_EXTERNAL_SCHEMA', 'sandbox')}.main_marks_student_for_click
...
def delete_from_click_deleted_records_from_pg(**context):
    ...
    try:
        ...
        for i in range(0, len(deleted_ids), batch_size):
            ...
            delete_query = """
            DELETE FROM main_marks_student 
            WHERE main_mark_id IN (%s)
...
def pg_to_ch_load_data_to_clickhouse(**context) -> None:
    ...
    try:
        ...
        for i in range(0, total_rows, batch_size):
            ...
            try:
                client.execute(
                    f"INSERT INTO main_marks_student ({','.join(full_columns)}) VALUES",
                    batch,
                    types_check=True
                )
                loaded = min(i + batch_size, total_rows)
        ...
        client.execute("OPTIMIZE TABLE main_marks_student FINAL")
...
with DAG(
    ...
    pg_to_pg_extract_task = PythonOperator(
        ...
        doc_md="Извлечение данных за указанный период из исходной таблицы main_marks_student PostgreSQL",
    )

    pg_to_pg_load_task = PythonOperator(
        task_id='pg_to_pg_load_data_to_target',
        python_callable=pg_to_pg_load_data_to_target,
        doc_md="Загрузка данных в целевую таблицу main_marks_student_for_click PostgreSQL",
    )
    ...
    pg_to_ch_extract_task = PythonOperator(
        ...
        doc_md="Извлечение данных из таблицы main_marks_student_for_click PostgreSQL",
    )
    ...
    pg_to_ch_load_task = PythonOperator(
        ...
        doc_md="Загрузка данных в таблицу main_marks_student ClickHouse",
    )
