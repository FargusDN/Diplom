    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            SELECT
                ...
                d.discipline_name,
                ct.class_type_name, 
                concat(ui.last_name, ' ', substring(ui.first_name,1, 1), substring(ui.middle_name,1, 1)) as "professor_fio",
                cs.auditorium,
                cs.create_dttm,
                cs.change_dttm
            FROM {os.getenv('POSTGRES_SOURCE_SCHEMA', 'public')}.class_schedule cs
            JOIN {os.getenv('POSTGRES_SOURCE_SCHEMA', 'public')}.user_info ui ON cs.professor_id = ui.user_id AND ui.user_status = 'Преподаватель'
            and coalesce(cs.change_dttm, cs.create_dttm) >= '{date1}'::timestamp AND coalesce(cs.change_dttm, cs.create_dttm) < '{date2}'::timestamp
            JOIN {os.getenv('POSTGRES_SOURCE_SCHEMA', 'public')}.study_group sg ON sg.group_id = cs.group_id
            JOIN {os.getenv('POSTGRES_SOURCE_SCHEMA', 'public')}.class_types ct ON ct.class_type_id = cs.class_type_id
            JOIN {os.getenv('POSTGRES_SOURCE_SCHEMA', 'public')}.disciplines d ON d.discipline_id = cs.discipline_id
            """
...
def pg_to_pg_load_data_to_target(**context) -> None:
    ...
    try:
        ...
        # Определяем колонки для обновления (все кроме class_schedule_id)
        update_columns = [col for col in columns if col != 'class_schedule_id']
        ...
        with conn.cursor() as cursor:
            ...
            upsert_query = f"""
            INSERT INTO {os.getenv('POSTGRES_TARGET_SCHEMA', 'sandbox')}.temp_class_schedule_for_click
            ({', '.join(columns)})
            VALUES ({', '.join(['%s'] * len(columns))})
            ON CONFLICT (class_schedule_id) DO UPDATE SET
            {update_set}
            """
            ...
            for i in range(0, total_rows, batch_size):
                ...
                try:
                    ...
                    # Попробуем обработать каждую строку отдельно для лучшего отслеживания ошибок
                    for row in batch:
                        try:
    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            SELECT
                ...
                create_dttm,
                change_dttm
            FROM {os.getenv('POSTGRES_EXTERNAL_SCHEMA', 'sandbox')}.temp_class_schedule_for_click
...
def delete_from_click_deleted_records_from_pg(**context):
    ...
    try:
        date1, date2 = get_dates(**context)
        logger.info(f"Using date range: {date1} to {date2}")
        # Получаем список ID записей, помеченных как удаленные в PostgreSQL
...
def pg_to_ch_load_data_to_clickhouse(**context) -> None:
    ...
    try:
        ...
        for i in range(0, total_rows, batch_size):
            ...
            try:
                client.execute(
                    f"INSERT INTO class_schedule ({','.join(full_columns)}) VALUES",
                    batch,
                    types_check=True
                )
                loaded = min(i + batch_size, total_rows)
        ...
        client.execute("OPTIMIZE TABLE class_schedule FINAL")
...
with DAG(
    ...
    pg_to_pg_extract_task = PythonOperator(
        ...
        doc_md="Извлечение данных за указанный период из исходной таблицы class_schedule PostgreSQL",
    )

    pg_to_pg_load_task = PythonOperator(
        task_id='pg_to_pg_load_data_to_target',
        python_callable=pg_to_pg_load_data_to_target,
        doc_md="Загрузка данных в целевую таблицу temp_class_schedule_for_click PostgreSQL",
    )
    ...
    pg_to_ch_extract_task = PythonOperator(
        ...
        doc_md="Извлечение данных из таблицы temp_class_schedule_for_click PostgreSQL",
    )
    ...
    pg_to_ch_load_task = PythonOperator(
        ...
        doc_md="Загрузка данных в таблицу class_schedule ClickHouse",
    )
