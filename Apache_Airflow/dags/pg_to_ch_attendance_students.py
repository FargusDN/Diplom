...
def get_dates(**context) -> tuple:
    ...
    # Способ 3: По умолчанию - текущий день
    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            select
                ...
                att.create_dttm,
                att.change_dttm
                from {os.getenv('POSTGRES_SOURCE_SCHEMA', 'public')}.attendance_students att 
                left join {os.getenv('POSTGRES_SOURCE_SCHEMA', 'public')}.class_schedule cs on cs.class_schedule_id = att.class_schedule_id
                left join {os.getenv('POSTGRES_SOURCE_SCHEMA', 'public')}.user_info ui on ui.user_id = att.student_id and ui.user_status = 'Студент'
                left join {os.getenv('POSTGRES_SOURCE_SCHEMA', 'public')}.disciplines d on cs.discipline_id = d.discipline_id
                left join {os.getenv('POSTGRES_SOURCE_SCHEMA', 'public')}.class_types ct on cs.class_type_id = ct.class_type_id
                left join {os.getenv('POSTGRES_SOURCE_SCHEMA', 'public')}.study_group sg on  ui.group_id = sg.group_id
                where coalesce(att.change_dttm, att.create_dttm) >= '{date1}'::timestamp 
              AND coalesce(att.change_dttm, att.create_dttm) < '{date2}'::timestamp
            """
...
def pg_to_pg_load_data_to_target(**context) -> None:
    ...
    try:
        ...
        # Определяем колонки для обновления (все кроме attendance_id)
        update_columns = [col for col in columns if col != 'attendance_id']
        ...
        with conn.cursor() as cursor:
            ...
            upsert_query = f"""
            INSERT INTO {os.getenv('POSTGRES_TARGET_SCHEMA', 'sandbox')}.attendance_students_for_click
            ({', '.join(columns)})
    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            SELECT
                ...
                create_dttm,
                change_dttm
            FROM {os.getenv('POSTGRES_EXTERNAL_SCHEMA', 'sandbox')}.attendance_students_for_click
    ...
    try:
        ...
        for row in result['data']:
            ...
            for i, value in enumerate(processed_row):
                if value is None:
                    # Для строковых полей устанавливаем пустую строку вместо NULL
                    if result['columns'][i] in ['discipline_name', 'class_type_name',
                                                'student_fio', 'group_name', 'reason_absent']:
                        processed_row[i] = ''
                    # Для boolean полей устанавливаем False
                    elif result['columns'][i] == 'attendance_bool':
                        processed_row[i] = False
                    # Для дат устанавливаем минимальную дату
                    elif result['columns'][i] in datetime_fields:
                        processed_row[i] = datetime(1970, 1, 1)
                    # Для числовых полей устанавливаем 0
                    elif result['columns'][i] == 'attendance_id':
                        processed_row[i] = 0
...
def pg_to_ch_load_data_to_clickhouse(**context) -> None:
    ...
    try:
        ...
        for i in range(0, total_rows, batch_size):
            ...
            try:
                client.execute(
                    f"INSERT INTO attendance_students ({','.join(full_columns)}) VALUES",
                    batch,
                    types_check=True
                )
                loaded = min(i + batch_size, total_rows)
        ...
        client.execute("OPTIMIZE TABLE attendance_students FINAL")
...
with DAG(
    ...
    pg_to_pg_extract_task = PythonOperator(
        ...
        doc_md="Извлечение данных за указанный период из исходной таблицы attendance_students PostgreSQL",
    )

    pg_to_pg_load_task = PythonOperator(
        task_id='pg_to_pg_load_data_to_target',
        python_callable=pg_to_pg_load_data_to_target,
        doc_md="Загрузка данных в целевую таблицу attendance_students_for_click PostgreSQL",
    )
    ...
    pg_to_ch_extract_task = PythonOperator(
        ...
        doc_md="Извлечение данных из таблицы attendance_students_for_click PostgreSQL",
    )
    ...
    pg_to_ch_load_task = PythonOperator(
        ...
        doc_md="Загрузка данных в таблицу attendance_students ClickHouse",
    )
